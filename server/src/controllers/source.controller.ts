...
function parseBulkSource(body: unknown) {
...
export async function listSource(req: Request, res: Response) {
  const { workspaceId } = parseWorkspaceId(req.params);
  const filters = parseListQuery(req.query);
  const source = await listSourcesForWorkspace(
    workspaceId,
    req.session.user.id,
    filters,
  );
...
export async function bulkDeleteSource(req: Request, res: Response) {
  ...
  const input = parseBulkSource(req.body);

  await bulkDeleteSourceForWorkspace(
    workspaceId,
    req.session.user.id,
    input.sourceIds,
  );
